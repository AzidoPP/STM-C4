...
- 字体按 7x5 顶部对齐显示（底部 1 行留空），避免底部贴边。
...
- 主逻辑采用 `MP3Sequence` 状态机（`main.c`），所有流程音频统一由状态机调度。
- 播放方式：按文件名播放，音频统一放在 TF 卡根目录 `MP3/` 文件夹。
- 固定播报编号：
  - 下包成功：`CONFIG_MP3_TRACK_ARM_SUCCESS`（默认 0101）
  - 纯爆炸：`CONFIG_MP3_TRACK_EXPLOSION`（默认 0102）
  - 拆弹成功：`CONFIG_MP3_TRACK_DEFUSE_SUCCESS`（默认 0103）
  - CT 胜利播报：`CONFIG_MP3_TRACK_CTWIN`（默认 0104）
  - T 胜利播报：`CONFIG_MP3_TRACK_TERWIN`（默认 0105）
- 音乐盒编号由配置控制：`CONFIG_MP3_CT_WIN_MUSICBOX_TRACK` / `CONFIG_MP3_T_WIN_MUSICBOX_TRACK`。

详细映射见：`TFCard_Files/README.md`
...
2. 上电音频：仅蜂鸣器（`CONFIG_STARTUP_BEEP_ENABLE` / `CONFIG_STARTUP_BEEP_ON_MS` / `CONFIG_STARTUP_BEEP_GAP_MS`），不播放 MP3。
3. 倒计时蜂鸣器频率：`CONFIG_BUZZER_COUNTDOWN_FREQ_HZ`。
4. 等待输入密码（长度 `CONFIG_PASSWORD_LEN`）。未输入时显示全 `+`；输入第一位后，未填位改为 `-`，数字从右向左累积。输入完成后延迟 `CONFIG_ARM_DELAY_MS` 再进入倒计时，避免误触。
...
倒计时时显示 `+++` 往返滚动；默认关闭数字倒计时，此时动画覆盖整行。开启数字倒计时后，`+++` 仅在左半区滚动，右半区显示 `mm:ss`。
...
2. **长按 # 手动拆弹**：长按超过 `CONFIG_LONG_PRESS_MS` 后开始拆弹，持续 `CONFIG_MANUAL_DEFUSE_MS` 即成功（`CONFIG_DEFUSE_ENABLE_MANUAL`）。
   - 过程显示：右端数字按 `CONFIG_DEFUSE_CYCLE_STEPS` 滚动，已确认的正确密码逐位向左累积；其余位以 `+` 和 `-` 交替闪烁。
3. **外部拆弹器拆弹**：PB3 触发后开始拆弹，持续 `CONFIG_EXTERNAL_DEFUSE_MS` 即成功（`CONFIG_DEFUSE_ENABLE_EXTERNAL`）。

拆弹成功后，LCD 按 `+++++++` → `-------` → 正确密码循环显示，每帧间隔由 `CONFIG_DEFUSE_BLINK_MS` 控制。

拆弹成功会进入 DEFUSE 音频序列：
- 若启用 `CONFIG_MP3_DEFUSE_STAGE_ENABLE`：
  - 若启用 `CONFIG_MP3_DEFUSE_SUCCESS_ENABLE`，播放 `CONFIG_MP3_TRACK_DEFUSE_SUCCESS` 并等待 `CONFIG_MP3_DEFUSE_SUCCESS_WAIT_MS`
  - 若启用 `CONFIG_MP3_CT_WIN_MUSICBOX_ENABLE`，播放 `CONFIG_MP3_CT_WIN_MUSICBOX_TRACK` 并等待 `CONFIG_MP3_CT_MUSICBOX_WAIT_MS`
  - 若启用 `CONFIG_MP3_CTWIN_ENABLE`，播放 `CONFIG_MP3_TRACK_CTWIN`

## 爆炸行为
- 倒计时结束未拆弹：继电器吸合。
- 播放 EXPLOSION 序列（按配置可启停各阶段）：
  - 若启用 `CONFIG_MP3_EXPLODED_STAGE_ENABLE` 且启用 `CONFIG_MP3_EXPLOSION_ENABLE`：
    - 启用 `CONFIG_MP3_T_WIN_MUSICBOX_ENABLE`：播放 T 音乐盒并等待 `CONFIG_MP3_T_MUSICBOX_WAIT_MS`
    - 未启用 T 音乐盒：播放纯爆炸音效并等待 `CONFIG_MP3_EXPLOSION_ONLY_WAIT_MS`
  - 若启用 `CONFIG_MP3_TERWIN_ENABLE`，播放 `CONFIG_MP3_TRACK_TERWIN`
...
## 配置入口
配置集中在 `Ver4.0/Keil_proj/c4/user/config.h`，用户说明在 `Ver4.0/config.md`。

新增运行时配置管理：
- `Ver4.0/Keil_proj/c4/user/config_manager.c`
- `Ver4.0/Keil_proj/c4/user/config_manager.h`

运行时配置机制要点：
- 程序启动时读取配置记录；若记录无效/版本不匹配/配置项数量不匹配，则回退到固件内默认值。
- 配置结构附带 magic / version / count / checksum，用于兼容新增配置项。
- 重新烧录后若配置结构变化，旧记录不会被继续使用。
- Keil + ST-Link 下载通常会擦除目标程序相关区域；是否整片擦除取决于下载配置，不保证每次整片擦除。

配置模式流程（上电按住 `#`）：
- 进入条件：上电/复位后的极短检测窗口内检测到已按住 `#`，并继续按住一小段时间后进入。
- 操作：输入配置编号 -> 输入新值 -> 保存确认。
- 特殊输入码：`090405`（固定 6 位）进入恢复默认确认流程。
- 保存或恢复成功后，停留提示并等待用户 `RESET`。

显示约束处理：
- LCD 仅 16 字符，配置模式统一使用短提示（如 `ID:` / `VAL:` / `SAVE?` / `RESET`）。
- 避免提示过长导致可读性下降。

倒计时显示新增：
- 新增 `CONFIG_DIGITAL_COUNTDOWN_ENABLE`。
- 开启后：左侧保持滚动动画，右侧 8 字符区域显示 `mm:ss`（前置 2 个空格）。
- 进入拆弹动画时自动隐藏数字倒计时，仅显示拆弹动画。

常用配置（宏）：
- 密码与上电：`CONFIG_PASSWORD_LEN` / `CONFIG_PASSWORD_COL` / `CONFIG_ARM_DELAY_MS` / `CONFIG_ARM_PRESET_ENABLE` / `CONFIG_ARM_PRESET_PASSWORD`
- 倒计时与滚动：`CONFIG_COUNTDOWN_MS` / `CONFIG_SCROLL_INTERVAL_MS` / `CONFIG_SCROLL_PATTERN_LEN`（预留，当前实现固定三个加号）
...
- MP3：
  - 基础：`CONFIG_MP3_VOLUME` / `CONFIG_MP3_ARM_SUCCESS_ENABLE`
  - 固定播报号：`CONFIG_MP3_TRACK_ARM_SUCCESS` / `CONFIG_MP3_TRACK_EXPLOSION` / `CONFIG_MP3_TRACK_DEFUSE_SUCCESS` / `CONFIG_MP3_TRACK_CTWIN` / `CONFIG_MP3_TRACK_TERWIN`
  - 阶段总开关：`CONFIG_MP3_DEFUSE_STAGE_ENABLE` / `CONFIG_MP3_EXPLODED_STAGE_ENABLE`
  - 子开关：`CONFIG_MP3_DEFUSE_SUCCESS_ENABLE` / `CONFIG_MP3_EXPLOSION_ENABLE` / `CONFIG_MP3_CT_WIN_MUSICBOX_ENABLE` / `CONFIG_MP3_T_WIN_MUSICBOX_ENABLE` / `CONFIG_MP3_CTWIN_ENABLE` / `CONFIG_MP3_TERWIN_ENABLE`
  - 音乐盒编号：`CONFIG_MP3_CT_WIN_MUSICBOX_TRACK` / `CONFIG_MP3_T_WIN_MUSICBOX_TRACK`
  - 等待时长：`CONFIG_MP3_DEFUSE_SUCCESS_WAIT_MS` / `CONFIG_MP3_CT_MUSICBOX_WAIT_MS` / `CONFIG_MP3_T_MUSICBOX_WAIT_MS` / `CONFIG_MP3_EXPLOSION_ONLY_WAIT_MS`
